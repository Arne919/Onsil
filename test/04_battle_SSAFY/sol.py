        ...
        for i in range(4):
            ...
            if 0 <= nx < N and 0 <= ny < N and not visited[nx][ny]:
                if map_data[nx][ny] == 'G':
                    ...
                    return arr
                elif 'E' in map_data[nx][ny] and allies['A'][3] != '0':
                    visited[nx][ny] = 1
                    map_data[nx][ny] = 'G'
                    allies['A'][3] = str(int(allies['A'][3]) - 1)
                    q.append((nx, ny, arr + [int_to_char[i] + ' F S']))
    return False
        ...
        for i in range(4):
            ...
            if 0 <= nx < N and 0 <= ny < N and not visited[nx][ny]:
                if map_data[nx][ny] == 'G':
                    visited[nx][ny] = 1
                    q.append((nx, ny, arr + [int_to_char[i] + ' A']))
                elif map_data[nx][ny] == 'F':
                    return arr

def caesar_brute_force_decrypt(ciphertext):
...
while game_data is not None:
    ...
    if result == False:
        result = reload(my_position[0], my_position[1])
        if codes:
            game_data = submit(f'G {caesar_brute_force_decrypt(codes[0])}')
            # game_data = submit(f'G {decode(codes[0])}')
            result = True
        else:
