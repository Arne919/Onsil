...
pygame.mixer.init()
pygame.mixer.music.load('assets/Shieldbearer.mp3')
...
def search(x, y):
    ...
    while q:
        ...
        for i in range(4):
            nx = x + dx[i]
            ny = y + dy[i]
            if 0 <= nx < N and 0 <= ny < N and not visited[nx][ny] and (our_tower_position[0] -5 <= nx <= our_tower_position[0] + 5 and our_tower_position[1] -3 <= ny <= our_tower_position[1] +3):
                if map_data[nx][ny] == 'G':
                    visited[nx][ny] = 1
                    q.append((nx, ny, arr + [int_to_char[i] + ' A']))
                elif 'E' in map_data[nx][ny]:
                    return arr + [int_to_char[i] + ' F S']
    return arr
    ...
    for i in range(len(map_data)):
        for j in range(len(map_data[0])):
            if map_data[i][j] == 'A':
                ...
                my_position[1] = j
            if map_data[i][j] == 'H':
                our_tower_position[0] = i
                our_tower_position[1] = j
