...
2. 게임
    ...
    - 게임 페이지:
        ...
        - 텍스트 입력 후 정답 제출 가능.
        - 결과 표시 (정답/오답, 영화 제목). 점수획득하기, 다시하기, 랭크페이지로 이동.
    - 게임 결과 페이지:
        - 맞춘 개수와 적립된 포인트 표시.
...
3. 랭크
    - 사용자 포인트 기반 랭킹 시스템:
        ...
        - 각 사용자 정보(프로필 사진, 이름, 랭크, 포인트, 게시물 수, 좋아요 수, 팔로워 수, 추천 영화) 표시.
...
- 추천 영화 표시:
    - 본인만의 추천 영화와 추천 이유 등록. 영화 누를 시 상세페이지 이동.
...
from rest_framework.authtoken.models import Token </br>
Token.objects.filter(user_id=2).delete() </br>
quit() </br>
py manage.py migrate </br>
python manage.py loaddata movies/fixtures/movies/popular.json movies/fixtures/movies/recent.json movies/fixtures/movies/upcoming.json movies/fixtures/movies/movie_data.json </br>
